...
--   and run it ONCE. Every statement is idempotent — re-running is always safe.
--
-- IDENTITY MODEL (launch fix):
--   players.id is a deterministic UUID derived from the Telegram user id on the
--   client, or a per-session temporary UUID when playing in a plain browser.
--   The table is therefore NOT foreign-keyed to auth.users (that FK made it
--   impossible to create Telegram-derived players and blocked every RPC with
--   a foreign-key violation). Telegram identity is still verified server-side
--   via initData HMAC in game_link_telegram, which binds telegram_id to the
--   player row. Anonymous auth is optional; gameplay never depends on it.
--
...
alter table public.players add column if not exists telegram_id text unique;
create index if not exists players_total_score_idx on public.players (total_score desc);

-- LAUNCH FIX: players are keyed by client-derived identities (Telegram hash or
-- web-session UUID). Any FK from players.id to auth.users prevents those rows
-- from ever existing and made every game RPC fail with a foreign-key violation.
do $$
declare
  c record;
begin
